...
const getLineCoords = (event) => {
  ...
  // SVG uses top-based coords, handpan center is 180px from bottom
  // Offsets are added (negative = higher up)
  return {
    x1: `calc(50% + ${targetPos.x}px)`,
    y1: `calc(100% - 180px + ${bottomY}px)`,
    x2: `calc(50% + ${targetPos.x}px)`,
    y2: `calc(100% - 180px + ${targetPos.y}px)`
  };
};
...
.target-glow {
  position: absolute;
  bottom: 180px; /* Aligned with handpan center (80px padding + ~180px radius) */
  left: 50%;
  border-radius: 50%;
  pointer-events: none;
  z-index: 8;
...
.note-bar {
  position: absolute;
  bottom: 180px; /* Reference point aligned with handpan center (80px padding + ~180px radius) */
  left: 50%;
